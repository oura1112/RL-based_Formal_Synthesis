        ...
        for i in range(session) :
            ...
            for e in range(episode_count):
                ...
                for step in range(step_count):
                    ...
                    #ambiguity setに基づくTの更新
                    gain_T = reward + max(self.Q[s_m_next][s_c_next][automaton_transit[2]][v_next]) #gamma -> step_count
                    estimated_T = self.T[s_m][s_c][automaton_transit[0]][v][event]
                    self.Nt[s_m][s_c][automaton_transit[0]][v][event] += 1
                    lammda_t = lammda_t + 1/(self.Nt[s_m][s_c][automaton_transit[0]][v][event] * math.log(1 + self.Nt[s_m][s_c][automaton_transit[0]][v][event])) * max(self.Q[s_m_next][s_c_next][automaton_transit[2]][v_next])
                    self.T[s_m][s_c][automaton_transit[0]][v][event] += 1/(self.Nt[s_m][s_c][automaton_transit[0]][v][event])*(gain_T - estimated_T - lammda_t)
                    ...
                    for control_pat_d in [control_pat]:
                        ...
                        if event in pi_d :
                            ...
                            #print(len(estimated_set))
                            #self.Q[s_m][s_c][automaton_transit[0]][v][control_pat_d] = self.prohibit_cost[control_pat_d] + estimated_set[0] #(1-1/self.Nq[s_m][s_c][automaton_transit[0]][v][control_pat_d])*self.Q[s_m][s_c][automaton_transit[0]][v][control_pat_d] + (1/self.Nq[s_m][s_c][automaton_transit[0]][v][control_pat_d])*(self.prohibit_cost[control_pat_d] + estimated_set[0]) #min(estimated_set)
                            lammda_q = lammda_q + 1/(self.Nq[s_m][s_c][automaton_transit[0]][v][control_pat_d] * math.log(1 + self.Nq[s_m][s_c][automaton_transit[0]][v][control_pat_d])) * estimated_set[0]
                            self.Q[s_m][s_c][automaton_transit[0]][v][control_pat_d] = (1-1/self.Nq[s_m][s_c][automaton_transit[0]][v][control_pat_d])*self.Q[s_m][s_c][automaton_transit[0]][v][control_pat_d] + (1/self.Nq[s_m][s_c][automaton_transit[0]][v][control_pat_d])*(self.prohibit_cost[automaton_transit[0]][control_pat_d] + estimated_set[0] - lammda_q) #min(estimated_set)
                            #print("comp")
                    """
                    #価値関数の更新　Q -> T
                    estimated = 0
