...
Let $\mathcal{E}(s) = \{ e \in E ; \text{the event $e$ is feasible at the state $s$} \}$.
Note that $\sum_{s' \in S} P_T(s'|s,e) = 1$ holds for any state $s \in S$ and any event $e \in \mathcal{E}(s) $, $P_T(s'|s,e) = 0$ for any $e \notin \mathcal{E}(s)$, and $\Sigma_{e \in \pi} P_E(e|s,\pi) = 1$ and we call the subset the control pattern.
...
\begin{definition}
  ...
  $S^{\otimes} = S \times \bar{X}$ is the finite set of states and we represent $s$ and $\bar{x}$ corresponding with $s^{\otimes} = (s,\bar{x}) \in S^{\otimes}$ as $\mysps$ and $\myspq$, respectively; $E^{\otimes}=E \cup \{ \varepsilon_{x^{\prime}} ; \exists x^{\prime} \text{s.t.} (x, \varepsilon_{x^{\prime}}, x^{\prime}) \in \delta \} $ is the finite set of events, where $\varepsilon_{x^{\prime}}$ is the action that represents an $\varepsilon$-transition to $x^{\prime} \in X$; $s_{init}^{\otimes} = (s_{init},\bar{x}_{init})$ is the initial states, $P^{\otimes}_T : S^{\otimes} \times S^{\otimes} \times E^{\otimes} \rightarrow [0,1]$ is the transition probability defined as
  \begin{align}
    P^{\otimes}_T(s^{\otimes \prime} | s^{\otimes}, e) =
    \left\{
    \begin{aligned}
      &P_T(s^{\prime} | s, e) &   &\text{if}\  (\bar{x}, L((s,e,s^{\prime})), \bar{x}^{\prime}) \in \bar{\delta}, a \in \mathcal{A}(s) \\
      &1 &   &\text{if}\ s=s^{\prime}, v=v^{\prime}, (x, \varepsilon_{x^{\prime}}, x^{\prime}) \in \delta, a= \varepsilon_{x^{\prime}} \\
      &0 &   &\text{otherwise} ,
    \end{aligned}
    \right. \nonumber
  \end{align}
